...
public class NilAuthenticationFailureHandler extends SimpleUrlAuthenticationFailureHandler {
	...
	public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
		...
		logger.info("登陆失败===>"+securityProperties.getBrowserProperties().getLoginType());
		if (LoginType.JSON.equals(securityProperties.getBrowserProperties().getLoginType())) {
			response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value());
			response.setContentType("application/json;charset=UTF-8");
//			response.getWriter().write(objectMapper.writeValueAsString(exception));
			response.getWriter().write(objectMapper.writeValueAsString(new SimpleResponse(exception.getMessage())));//只返回错误的信息，不返回所有的异常信息
